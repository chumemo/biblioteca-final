...
class BusquedaController extends Controller
{
    public function index(Request $request) {
        ...
        // Buscamos por tema  
        $resultsCapsulas = Capsula::where('temas', 'like', "%$query%")
            ->where('estado', 1)
            ->get()->each(function ($item) {
            $item->tipo = 1;
            $item->link = route('capsula.show', $item->id);
        });
        $resultsCatalogos = Catalogo::where('temas', 'like', "%$query%")
            ->where('estado', 1)
            ->get()->each(function ($item) {
            $item->tipo = 2;
            $item->link = route('catalogo.show', $item->id);
        });
        $resultsCompendios = Compendio::where('temas', 'like', "%$query%")
            ->where('estado', 1)
            ->get()->each(function ($item) {
            $item->tipo = 3;
            $item->link = route('compendio.show', $item->id);
        });
        $resultsFolletos = Folleto::where('temas', 'like', "%$query%")
            ->where('estado', 1)
            ->get()->each(function ($item) {
            $item->tipo = 4;
            $item->link = route('folleto.show', $item->id);
        });
        $resultsDocumentos = Documento::where('temas', 'like', "%$query%")
            ->where('estado', 1)
            ->get()->each(function ($item) {
            $item->tipo = 5;
            $item->link = route('documento.show', $item->id);
        });
        $resultsFormatos = Formato::where('temas', 'like', "%$query%")
            ->where('estado', 1)
            ->get()->each(function ($item) {
            $item->tipo = 6;
            $item->link = route('formato.show', $item->id);
        });
        $resultsManuales = Manual::where('temas', 'like', "%$query%")
            ->where('estado', 1)
            ->get()->each(function ($item) {
            $item->tipo = 7;
            $item->link = route('manual.show', $item->id);
